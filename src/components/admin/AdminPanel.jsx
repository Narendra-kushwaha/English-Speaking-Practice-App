  ...
  return (
    <div style={S.pg}><div style={S.wrap}>
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:16, flexWrap:"wrap", gap:10 }}>
        ...
        <Btn onClick={logoutUser} color="#EF4444" sm>Logout</Btn>
      </div>
      ...
      <div style={{ background:"linear-gradient(135deg,#F59E0B22,#FCD34D11)", border:"1.5px solid #F59E0B44", borderRadius:14, padding:"16px 20px", marginBottom:20 }}>
        <div style={{ fontSize:10, color:"#F59E0B", fontWeight:700, letterSpacing:1, marginBottom:6 }}>🔑 YOUR PERMANENT ADMIN ID</div>
        <div style={{ display:"flex", alignItems:"center", gap:12, flexWrap:"wrap" }}>
          <div style={{ fontSize:24, fontWeight:900, color:"#FCD34D", letterSpacing:3 }}>{profile?.adminId}</div>
          <span style={{ color:"#94A3B8", fontSize:12 }}>Share this with students so they can register under you</span>
        </div>
      </div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(200px,1fr))", gap:14 }}>
        {[
          ...
          <button key={m.key} onClick={() => setScreen(m.key)} style={{
            padding:"22px 18px", borderRadius:14, border:"1.5px solid #F59E0B33",
            background:"#1E293B", cursor:"pointer", textAlign:"left", width:"100%", boxSizing:"border-box",
          }}
